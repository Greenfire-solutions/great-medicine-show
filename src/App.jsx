...
function App() {
  ...
  const introClickAudioRef = useRef(null)
  ...
  const handleStartIntro = () => {
    if (introClickAudioRef.current) {
      introClickAudioRef.current.currentTime = 0
      introClickAudioRef.current.play().catch(() => {})
    }
  ...
  useEffect(() => {
    introClickAudioRef.current = new Audio()
    return () => {
  ...
  return (
    <div className="app-shell arcade-screen">
      {(showIntro || introFadingOut) && (
        <section className={`intro-screen ${introFadingOut ? 'fade-out' : ''}`} aria-label="Game intro">
          ...
          <div className="intro-content">
            ...
            />
            <button className="intro-start-button" onClick={handleStartIntro} aria-label="Start game">
              <img
      ...
      <div className={`app-main ${showIntro ? 'app-main-hidden' : ''}`}>
