...
function App() {
  ...
  const mapNavigationEntries = [
    { id: 'hall', name: 'Hall of Great Works', type: 'map', targetMap: 'hall' },
    { id: 'civilx', name: 'CivilX Lab', type: 'map', targetMap: 'civilx' },
    { id: 'fire', name: 'Fire Temple', type: 'map', targetMap: 'fireTemple' },
    { id: 'water', name: 'Water Temple', type: 'map', targetMap: 'waterTemple' },
    { id: 'earth', name: 'Earth Temple', type: 'map', targetMap: 'earthTemple' },
    { id: 'air', name: 'Air Temple', type: 'map', targetMap: 'airTemple' },
    { id: 'course', name: 'Course Academy', type: 'page' },
    ...(currentMap !== 'overworld' ? [{ id: 'back-overworld', name: 'Back to Overworld', type: 'map', targetMap: 'overworld' }] : [])
  ]

  const locationDescriptions = {
    'Hall of Great Works': 'The central hub where music, store, shows, and courses connect.',
    'CivilX Lab': 'A research enclave for advanced prototypes, quests, and experiments.',
    'Fire Temple': 'Elemental forge of action, courage, and focused transformation.',
    'Water Temple': 'A flowing sanctuary of intuition, healing, and reflection.',
    'Earth Temple': 'Grounded chamber for structure, roots, and regenerative vision.',
    'Air Temple': 'A high-vibration domain for clarity, strategy, and communication.',
    'Course Academy': 'Training grounds for creative systems, production, and growth.',
    'Back to Overworld': 'Return to the wider world map and travel to other locations.'
  }
  ...
  const selectedLocation = useMemo(
    () =>
      currentMapDef.nodes.find((node) => node.name === activeSection) ||
      mapNavigationEntries.find((entry) => entry.name === activeSection) || {
        id: currentMap,
        name: currentMapDef.title
      },
    [activeSection, currentMap, currentMapDef, mapNavigationEntries]
  )
  ...
  return (
    <div className={`app-shell arcade-screen ${step < 4 ? 'onboarding-mode' : ''}`}>
      {step === 4 && (
        <section className="world-panel rpg-shell arcade-panel">
          <button className="arcade-button nav-toggle rpg-nav-toggle" onClick={() => setNavOpen((value) => !value)}>
            {navOpen ? 'Hide Navigation' : 'Show Navigation'}
          </button>

          <header className="banner rpg-top-banner arcade-panel">
            <p className="kicker">GREAT MEDICINE SHOW RPG</p>
            <h1 className="glyph-title">HALL OF GREAT WORKS</h1>
          </header>

          <aside className={`sidebar arcade-panel rpg-nav-panel ${navOpen ? 'open' : ''}`}>
            <h3 className="glyph-title">Map Navigation</h3>
            <p className="small">Current Location</p>
            <p className="current-location">{currentMapDef.title}</p>
            <div className="glowing-divider" />
            {mapNavigationEntries.map((section) => (
              <button
                key={section.id}
                className={`arcade-button linkish nav-item ${activeSection === section.name ? 'selected' : ''}`}
                onClick={() => {
                  navigateToNode(section)
                  setNavOpen(false)
                }}
              >
                <span className="nav-icon-slot" aria-hidden="true">{navIcons[section.name] || '✦'}</span>
                <span>{section.name}</span>
              </button>
            ))}
          </aside>

          <div className="world-content rpg-world-content">
            ...
            <div className="rpg-main-frame">
              <h2 className="glyph-title">{currentMapDef.title}</h2>
              <div className="glowing-divider" />

              <div className={`rpg-map pixel-map ${currentMap === 'hall' ? 'map-hall' : ''} ${currentMap === 'overworld' ? 'map-overworld' : ''}`} role="group" aria-label="2D RPG world map">
            ...
            <article className="arcade-panel section-panel rpg-info-panel">
              <div>
                <p className="small">Selected Location</p>
                <h3 className="glyph-title">{selectedLocation.name}</h3>
                <p className="arcade-copy">
                  {locationDescriptions[selectedLocation.name] || locationDescriptions[activeSection] || `Explore ${selectedLocation.name} for quests, events, and interaction prompts.`}
                </p>
              </div>
              <div className="info-thumbnail" aria-hidden="true">
                <span>Thumbnail</span>
              </div>
