...
import { upcomingShows, bookingInquiryUrl, bookingOffers } from './data/showsData'
                ...
                <div className="music-shell">
                  ...
                  <div>
                    ...
                    <div className="platform-links">
                      {[
                        ['Spotify', activeTrack.spotifyUrl],
                        ['SoundCloud', activeTrack.soundcloudUrl],
                        ['Bandcamp', activeTrack.bandcampUrl],
                        ['YouTube', activeTrack.youtubeUrl]
                      ]
                        .filter(([, url]) => Boolean(url))
                        .map(([platform, url]) => (
                          <a key={platform} className="arcade-button" href={url} target="_blank" rel="noreferrer">
                            {platform}
                          </a>
                        ))}
                    </div>
                    <div className="track-selector">
                      {musicTracks.map((track, index) => (
                        <button
                          key={track.id}
                          className={`arcade-button ${activeTrackIndex === index ? 'selected' : ''}`}
              ...
              <>
                ...
                <div className="overlay-grid">
                  {upcomingShows.map((show) => (
                    <article key={show.id} className="overlay-card">
                      <h4>{show.title}</h4>
                ...
                <div className="overlay-grid">
                  {bookingOffers.map((offer) => (
                    <article key={offer.id} className="overlay-card">
                      ...
                      {offer.bookingUrl ? (
                        <a className="arcade-button" href={offer.bookingUrl} target="_blank" rel="noreferrer">
                          Book Offer
                        </a>
                      ) : null}
                    </article>
                ...
                {bookingInquiryUrl ? (
                  <a className="arcade-button" href={bookingInquiryUrl} target="_blank" rel="noreferrer">
