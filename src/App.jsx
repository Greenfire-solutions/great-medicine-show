...
function App() {
  ...
  const submitBookingForm = (event) => {
    ...
    const bookingType = bookingForm.bookingType
    const subject = `Booking Inquiry: ${bookingType}`
    const body = [
      `Name: ${bookingForm.name}`,
      `Email: ${bookingForm.email}`,
      `Phone: ${bookingForm.phone}`,
      `Booking Type: ${bookingForm.bookingType}`,
      `Event Date: ${bookingForm.eventDate}`,
      `Event Location: ${bookingForm.eventLocation}`,
      `Estimated Budget: ${bookingForm.estimatedBudget}`,
      '',
      'Message / Details:',
      bookingForm.message
    ].join('\n')

    const mailtoUrl = `mailto:civilizationexplorer@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`
      ...
      {bookingForm && (
        <div className="booking-modal-backdrop" role="dialog" aria-label="Booking inquiry form">
          <form className="booking-modal arcade-panel" onSubmit={submitBookingForm}>
            ...
            <p className="small">Your email app will open so you can review and send the inquiry.</p>
            ...
            <div className="dialogue-actions">
              <button type="submit" className="arcade-button">Open Email Draft</button>
